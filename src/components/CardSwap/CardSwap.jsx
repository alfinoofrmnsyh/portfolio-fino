...
const CardSwap = forwardRef(({
  ...
  // LOGIKA SWAP (Dipisah agar bisa dipanggil manual)
  const swap = () => {
    if (order.current.length < 2) return;
    if (tlRef.current && tlRef.current.isActive()) return; // Mencegah klik double saat animasi jalan
  ...
  // --- BAGIAN KUNCI: Ekspos fungsi ke Parent (App.jsx) ---
  useImperativeHandle(ref, () => ({
    next: () => {
      swap();
    },
    prev: () => {
      // Logika untuk swap mundur (opsional sederhana: pindahkan array belakang ke depan)
      ...
      // Re-place posisi secara instan lalu jalankan animasi normal
